...
abstract class SubtagSet
  ...
  public boolean contains(Subtag subtag) {
    for (Subtag tag : this) 
      if (tag.equals(subtag)) return true;
    return false;
  }
  ...
  public boolean isValid() {
    for (Subtag subtag : this) 
      if (!subtag.isValid()) return false;
    return true;
  }
  ...
  public Subtag[] toArray() {
    ...
    for (Subtag tag : this) tags.add(tag);
  ...
  public int compareTo(SubtagSet o) {
    ...
    if (e.hasNext() && !i.hasNext()) return -1;
    if (i.hasNext() && !e.hasNext()) return 1;
    return 0;
  }
