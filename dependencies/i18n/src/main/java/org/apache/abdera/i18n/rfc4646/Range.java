...
public class Range  
  ...
  public boolean matches(Lang lang, boolean extended) {
    ...
    if (isBasic() && !extended) {
      if (primary.getType() == Subtag.Type.WILDCARD) return true;
      for(;i.hasNext() && e.hasNext();) {
        Subtag in = i.next();
        Subtag en = e.next();
        if (!in.equals(en)) return false;
      }
      return true;
    } else {
      Subtag icurrent = i.next();
      Subtag ecurrent = e.next();
      if (!icurrent.equals(ecurrent)) return false;
  ...
  public static Range parse(String range, boolean extended) {
    if (!extended) {
      ...
      if (m.find()) {
        ...
        primary = 
          new Subtag(
            first.equals("*") ? Subtag.Type.WILDCARD : Subtag.Type.SIMPLE,
            first.toLowerCase(Locale.US));
        current = primary;
        ...
        while(n.find()) {
          ...
          current = 
            new Subtag(
              name.equals("*") ? Subtag.Type.WILDCARD : Subtag.Type.SIMPLE,
              name,
              current);
        }
      }
      ...
      if (m.find()) {
        ...
        if (script != null && script.length() > 0)
          current = new Subtag(script.substring(1).equals("*") ? Type.WILDCARD : Type.SCRIPT, script.substring(1), current);
        if (region != null && region.length() > 0)
          current = new Subtag(region.substring(1).equals("*") ? Type.WILDCARD : Type.REGION, region.substring(1), current);
        ...
        if (extension != null && extension.length() > 0) {
          ...
          for (int i = 1; i < tags.length; i++) {
            String tag = tags[i];
            current = new Subtag(tag.equals("*") ? Type.WILDCARD : tag.length() == 1 ? Type.SINGLETON : Type.EXTENSION, tag, current);
          }
        }
